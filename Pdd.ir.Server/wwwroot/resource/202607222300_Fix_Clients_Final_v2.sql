...
-- Step 4: Remove old columns with constraints
DECLARE @sql NVARCHAR(MAX);

-- Drop Icon constraint first
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'Icon')
BEGIN
    SET @sql = '';
    SELECT @sql = @sql + 'ALTER TABLE Clients DROP CONSTRAINT ' + name + '; '
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('Clients')
    AND parent_column_id = (SELECT column_id FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'Icon');
...
BEGIN
    ...
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('Clients')
    AND parent_column_id = (SELECT column_id FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'Color');
...
-- Drop ImageUrl if it exists (from old migration)
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'ImageUrl')
BEGIN
    -- Only drop if we just added it (check if it has default constraint)
    SET @sql = '';
    SELECT @sql = @sql + 'ALTER TABLE Clients DROP CONSTRAINT ' + name + '; '
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('Clients')
    AND parent_column_id = (SELECT column_id FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'ImageUrl');
    
    IF @sql <> '' EXEC(@sql);
    
    ALTER TABLE Clients DROP COLUMN ImageUrl;
    PRINT '✅ ImageUrl column removed (old)';
END

-- Drop ImageData if it exists
...
-- Step 5: Re-add ImageUrl if needed
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Clients') AND name = 'ImageUrl')
BEGIN
    ALTER TABLE Clients ADD ImageUrl NVARCHAR(500) NOT NULL DEFAULT '';
    PRINT '✅ ImageUrl column added';
...
-- Step 6: Insert default data if empty
